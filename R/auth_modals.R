# Modal-based signup and login (pl-contest).
...
#
# PL schema: no username. Accounts are email + display name; login
# accepts either (verify_login disambiguates on the presence of "@").
...
show_signup_modal <- function() {
  showModal(modalDialog(
    ...
    tagList(
      ...
      helpText("Your display name is shown on the leaderboard and can be
                used to log in. 2\u201350 characters, no @ symbol. It cannot
                be changed later."),
...
show_login_modal <- function() {
  showModal(modalDialog(
    title = "Log in",
    tagList(
      textInput("mod_login_identifier", "Email or display name"),
...
auth_modals_server_logic <- function(input, output, session, current_user) {
  ...
  observeEvent(input$mod_signup_submit, {
    ...
    if (is.null(result)) {
      ...
      user <- verify_login(input$mod_signup_email, input$mod_signup_password)
      current_user(user)
      removeModal()
    } else {
  ...
  observeEvent(input$mod_login_submit, {
    ...
    user <- verify_login(input$mod_login_identifier, input$mod_login_password)

    if (is.null(user)) {
      message("    login failed")
      output$mod_login_message <- renderUI(
        div("Invalid login details.", class = "register-error")
      )
    } else {
      message("    login succeeded for ", user$display_name)
      current_user(user)
      removeModal()
    }
  })
  ...
  observeEvent(input$mod_chpw_submit, {
    ...
    # 1. Verify the current password (email is the stable identifier).
    check <- verify_login(user$email, input$mod_chpw_current)
    if (is.null(check)) {
    ...
    result <- update_user_password(user$email, input$mod_chpw_new)

    if (is.null(result)) {
      message("    chpw succeeded for ", user$display_name)
      removeModal()
